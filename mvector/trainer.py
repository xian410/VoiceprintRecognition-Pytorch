...
class MVectorTrainer(object):
    ...
    def __setup_dataloader(self, is_train=False):
        ...
        if is_train:
            self.train_dataset = MVectorDataset(
            ...
            if (
                self.configs.dataset_conf.get("is_use_pksampler", False)
                or use_loss == "TripletAngularMarginLoss"
            ):
                # 设置支持多卡训练
        ...
        # 获取评估的注册数据和检验数据
        self.enroll_dataset = MVectorDataset(
            data_list_path=self.configs.dataset_conf.enroll_list,
            audio_featurizer=self.audio_featurizer,
            mode="eval",
            **dataset_args,
        )
        self.enroll_loader = DataLoader(
            dataset=self.enroll_dataset,
            collate_fn=collate_fn,
            shuffle=False,
            batch_size=self.configs.dataset_conf.eval_conf.batch_size,
            **data_loader_args,
        )
        self.trials_dataset = MVectorDataset(
            data_list_path=self.configs.dataset_conf.trials_list,
            audio_featurizer=self.audio_featurizer,
            mode="eval",
            **dataset_args,
        )
        self.trials_loader = DataLoader(
            dataset=self.trials_dataset,
            collate_fn=collate_fn,
            shuffle=False,
            batch_size=self.configs.dataset_conf.eval_conf.batch_size,
            **data_loader_args,
    ...
    def extract_features(self, save_dir="dataset/features", max_duration=100):
        ...
        for data_list in [
            self.configs.dataset_conf.train_list,
                ...
                for features, labels, input_lens in tqdm(test_loader):
                    for i in range(len(features)):
                        ...
                        save_path = os.path.join(
                            save_dir, str(label), f"{int(time.time() * 1000)}.npy"
                        ).replace("\\", "/")
                        os.makedirs(os.path.dirname(save_path), exist_ok=True)
                        np.save(save_path, feature)
                        f.write(f"{save_path}\t{label}\n")
            logger.info(
                f"{data_list}列表中的数据已提取特征完成，新列表为：{save_data_list}"
            )
    ...
    def __setup_model(self, input_size, is_train=False):
        ...
        if is_train:
            ...
            # 分类器
            classifier = SpeakerIdentification(
                input_dim=self.backbone.embd_dim, **self.configs.model_conf.classifier
            )
            # 合并模型
            ...
            if self.configs.loss_conf.get("use_margin_scheduler", False):
                margin_scheduler_args = dict(
                    increase_start_epoch=int(self.configs.train_conf.max_epoch * 0.3),
                    fix_epoch=int(self.configs.train_conf.max_epoch * 0.7),
                    initial_margin=0.0,
                    final_margin=0.3,
                )
                margin_scheduler_args.update(
                    self.configs.loss_conf.get("margin_scheduler_args", {})
                )
                self.margin_scheduler = MarginScheduler(
                    criterion=self.loss,
        ...
        # 使用Pytorch2.0的编译器
        if (
            self.configs.train_conf.use_compile
        ...
        for batch_id, (features, label, input_len) in enumerate(self.train_loader):
            ...
            if use_loss == "SubCenterLoss":
                ...
                cosine = torch.reshape(
                    logits, (-1, logits.shape[1] // loss_args.K, loss_args.K)
                )
            ...
            if batch_id % self.configs.train_conf.log_interval == 0 and local_rank == 0:
                ...
                # 记录学习率
                writer.add_scalar(
                    "Train/lr", self.scheduler.get_last_lr()[0], self.train_log_step
                )
                if self.margin_scheduler:
                    writer.add_scalar(
                        "Train/margin",
                        self.margin_scheduler.get_margin(),
                        self.train_log_step,
                    )
            ...
            if batch_id % 10000 == 0 and batch_id != 0 and local_rank == 0:
                save_checkpoint(
                    ...
                    margin_scheduler=self.margin_scheduler,
    ...
    def train(
        ...
        self.model = load_pretrained(
            model=self.model, pretrained_model=pretrained_model
        )
        # 加载恢复模型
        (
            self.model,
        ...
        for epoch_id in range(last_epoch, self.configs.train_conf.max_epoch):
            ...
            if local_rank == 0 and do_eval:
                ...
                logger.info(
                    "Test epoch: {}, time/epoch: {}, threshold: {:.2f}, EER: {:.5f}, "
                    "MinDCF: {:.5f}".format(
                        epoch_id,
                ...
                logger.info("=" * 70)
                writer.add_scalar(
                    "Test/threshold", self.eval_threshold, self.test_log_step
                )
    ...
    def evaluate(self, resume_model=None, save_image_path=None):
        ...
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            eval_model = (
                self.model.module
                if len(self.model.module) == 1
                else self.model.module[0]
            )
        else:
        ...
        with torch.no_grad():
            for batch_id, (audio_features, label, input_len) in enumerate(
                ...
                # 存放特征
                enroll_features = (
                    np.concatenate((enroll_features, feature))
                    if enroll_features is not None
                    else feature
                )
                enroll_labels = (
                    np.concatenate((enroll_labels, label))
        ...
        with torch.no_grad():
            for batch_id, (audio_features, label, input_lens) in enumerate(
                ...
                # 存放特征
                trials_features = (
                    np.concatenate((trials_features, feature))
                    if trials_features is not None
                    else feature
                )
                trials_labels = (
                    np.concatenate((trials_labels, label))
        ...
        for i in tqdm(range(len(trials_features)), desc="特征对比"):
            ...
            score = (
                cosine_similarity(trials_feature, enroll_features)
                .astype(np.float32)
                .tolist()[0]
            )
            trials_label = np.expand_dims(trials_labels[i], 0).repeat(
                len(enroll_features), axis=0
            )
        ...
        if save_image_path:
            ...
            plt.text(
                threshold,
                fpr[index],
                (round(threshold, 3), round(fpr[index], 5)),
                color="red",
            )
            ...
            logger.info(
                f"结果图以保存在：{os.path.join(save_image_path, 'result.png')}"
            )
    ...
    def export(
        self,
        save_model_path="models/",
        resume_model="models/CAMPPlus_Fbank/best_model/",
    ):
        """
        导出预测模型
        :param save_model_path: 模型保存的路径
