...
results = predictor.speaker_diarization(
    args.audio_path, speaker_num=args.speaker_num, search_audio_db=args.search_audio_db
)
saved_path = os.path.join("output", os.path.basename(args.audio_path) + ".txt")
print(f"识别结果保存至 {os.path.abspath(saved_path)}")
